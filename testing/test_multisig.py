...
def import_multisig(request, is_q1, need_keypress, offer_ms_import, press_nfc):
    def doit(fname=None, way="sd", data=None, name=None):
        ...
        if way in (None, "usb"):  # USB
            ...
            # Put the file on the card before the picker opens. The device
            # scans the card when Import is chosen, so writing it afterwards
            # leaves the picker with nothing to show and it drops back to the
            # menu -- which any test that wipes the card first hits every time.
