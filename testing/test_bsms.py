...
from pycoin.contrib.msg_signing import verify_message, sign_message
from pycoin.encoding import from_bytes_32, double_sha256
from pycoin.ui import address_for_pay_to_script, address_for_pay_to_script_wit
...
def make_signer_round1(settings_get, settings_set, settings_remove, microsd_path, virtdisk_path):
    def doit(token, way, root_xprv=None, bsms_version=BSMS_VERSION, description=None, purge_bsms=True,
        ...
        data += "%s" % description
        preimage = b'\x18Bitcoin Signed Message:\n' + ser_compact_size(len(data)) + data.encode()
        md = double_sha256(preimage)
        if wrong_sig:
            # hash again to get wrong sig
            md = double_sha256(md)
        md = from_bytes_32(md)
        sig = sign_message(sk, msg_hash=md)
        data += "\n%s" % sig
        suffix = ".txt"
        mode = "wt"
        if token != "00":
...
def ms_address_from_descriptor_pycoin(desc_obj: MultisigDescriptor, subpath="0/0", network="XTN"):
    nodes = [
        BIP32Node.from_hwif(ek).subkey_for_path(subpath)
        for _, _, ek in desc_obj.keys
    ]
    secs = [node.sec() for node in nodes]
    secs.sort()
    ms_script = ScriptMultisig(desc_obj.M, secs).script()
    if desc_obj.addr_fmt == af_map["p2wsh"]:
        address = address_for_pay_to_script_wit(ms_script, network)
    else:
        # wrapped segwith in p2sh
        s256 = hashlib.sha256(ms_script).digest()
        ws = b"\x00" + len(s256).to_bytes(1, 'little') + s256
        address = address_for_pay_to_script(ws, network)
    return address
...
def test_coordinator_round1(way, encryption_type, M_N, addr_fmt, clear_ms, goto_home, need_keypress, pick_menu_item,
                            ...
                            settings_get, virtdisk_wipe, microsd_wipe):
    M, N = M_N
    virtdisk_wipe()
    microsd_wipe()
    settings_remove(BSMS_SETTINGS)  # clear bsms
    ...
    if encryption_type == "3":
        ...
        if way == "sd":
            ...
            if "press (3) to share via NFC" not in story:
                ...
                time.sleep(0.2)
                need_keypress("y")  # exit NFC UI simulation
                time.sleep(0.5)
        else:
            # virtual disk
...
def test_signer_round1(way, encryption_type, M_N, addr_fmt, clear_ms, goto_home, need_keypress, pick_menu_item, cap_menu,
                       ...
                       make_coordinator_round1, nfc_write_text, virtdisk_wipe, microsd_wipe):
    M, N = M_N
    virtdisk_wipe()
    microsd_wipe()
    ...
    if encryption_type == "3":
        ...
        if way != "nfc":
            time.sleep(0.2)
            _, story = cap_story()
            assert ('Select file containing the token to be imported. File extension has to be ".token" '
                    'and file has to contain single line with hex encoded token string.') in story
            need_keypress("y")
    ...
    # verify msg pycoin
    assert verify_message(sk, sig, signed_data)
...
def test_coordinator_round2(way, encryption_type, M_N, addr_fmt, auto_collect, clear_ms, goto_home, need_keypress,
                            ...
                            virtdisk_wipe, microsd_wipe, pick_menu_item):
    def get_token(index):
    ...
    if way == "nfc":
        ...
        if auto_collect:
            ...
            need_keypress("y")  # continue with manual selection
    ...
    if way == "sd":
        ...
        if "(3) to share via NFC" not in story:
            pytest.skip("NFC disabled")
        else:
            need_keypress("3")
    else:
    ...
    if encryption_type == "2":
        ...
        for token, desc_template in zip(tokens, descriptor_templates):
            plaintext = decrypt(key_derivation_function(token), desc_template.hex())
            assert plaintext
        ...
        plaintext = decrypt(key_derivation_function(get_token(0)), descriptor_templates[0].hex())
        assert plaintext
        the_template = plaintext
    else:
        assert len(descriptor_templates) == 1
    ...
    # pycoin test ms address
    address = ms_address_from_descriptor_pycoin(desc_obj)
    assert addr == address
...
def test_invalid_token_signer_round1(token, way, pick_menu_item, cap_story, need_keypress, nfc_write_text, microsd_path,
                                     virtdisk_path, goto_home):
    goto_home()
    pick_menu_item('Settings')
    pick_menu_item('Multisig Wallets')
    pick_menu_item('BSMS (BIP-129)')
...
def test_failure_coordinator_round2(encryption_type, make_coordinator_round1, make_signer_round1, microsd_wipe, cap_menu,
                                    virtdisk_wipe, pick_menu_item, need_keypress, goto_home, cap_story, failure):
    virtdisk_wipe()
    microsd_wipe()
...
def test_failure_signer_round2(encryption_type, goto_home, need_keypress, pick_menu_item, cap_menu, cap_story,
                               ...
                               make_coordinator_round2, virtdisk_wipe, failure):
    virtdisk_wipe()
    microsd_wipe()
...
def test_integration_signer(encryption_type, M_N, addr_fmt, clear_ms, microsd_wipe, goto_home, pick_menu_item, cap_story,
                            need_keypress, settings_remove, microsd_path, settings_get, cap_menu, use_mainnet):
