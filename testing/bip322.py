...
def bip322_from_classic_tx(psbt, msg=b"POR"):
    # Convert an existing PSBT into BIP-322 form while preserving edge-only
    # miniscript, tapscript, and MuSig input metadata.
    ...
    if po.inputs[0].utxo:
        ...
        script_pubkey = i0_wutxo.scriptPubKey

    to_spend = CTransaction()
    to_spend.nVersion = 0
    msg_hash = bip322_msg_hash(msg)
    to_spend.vin = [CTxIn(COutPoint(hash=0, n=0xffffffff),
                          scriptSig=b'\x00\x20' + msg_hash)]
    ...
    rv = BytesIO()
    po.serialize(rv)
    return rv.getvalue(), script_pubkey
