...
#
# test_silentpayments.py - Tests for BIP-375 Silent Payments implementation
#
...
from sp_helpers import (
    _sim_validate_sp, _sim_get_ecdh_and_pubkey, _sim_get_outpoints,
    _sim_get_combined_privkey, _sim_get_input_pubkey, _sim_verify_dleq,
    _sim_compute_ecdh_share, _sim_compute_output_script,
)
...
def _get_vector(category, description_substr):
    ...
    assert len(matches) == 1, \
        "Expected 1 match for %r in %s, got %d" % (description_substr, category, len(matches))
    return matches[0]
...
class TestBIP375InvalidVectors:
    ...
    # Vectors where the firmware raises FatalPSBTIssue — parametrized for easy extension.
    ...
    _SIM_INVALID = [
        ...
        ('missing PSBT_OUT_SCRIPT field when sending',          'either PSBT_OUT_SCRIPT or PSBT_OUT_SP_V0_INFO'),
        ('empty PSBT_OUT_SCRIPT field when sending',            'either PSBT_OUT_SCRIPT or PSBT_OUT_SP_V0_INFO'),
        ('ineligible P2MS input',                               'no ECDH share for scan key'),
    ...
    def test_invalid_vectors_sim_validate_sp(self, sim_exec, sim_execfile, desc, expected_err):
        ...
        assert expected_err in err, \
            "Expected %r in error for [%s], got: %s" % (expected_err, desc, err)
...
class TestBIP375ValidVectors:
    ...
    def test_valid_ecdh_computation(self, sim_exec, sim_execfile):
        vectors = load_test_vectors()['valid']
        for vi in range(13):
            vec = vectors[vi]
            p = BasicPSBT().parse(vec['psbt'].encode())
            combined_privkey = _sim_get_combined_privkey(sim_exec, sim_execfile, p, vec)
            ...
            for expected in vec['supplementary']['sp_proofs']:
                ...
                if 'input_index' in expected:
                    ik = next(k for k in vec['supplementary']['inputs']
                             if k['input_index'] == expected['input_index'])
                    privkey_hex = ik['private_key']
                else:
                    privkey_hex = '%064x' % combined_privkey
                actual = _sim_compute_ecdh_share(sim_exec, sim_execfile, privkey_hex, scan_key)
                assert actual == expected_ecdh, \
                    "ECDH mismatch in valid[%d] for scan_key %s" % (vi, expected['scan_key'][:16])
    ...
    def test_valid_dleq_verification(self, sim_exec, sim_execfile):
        ...
        for vi in range(13):
            ...
            for scan_key, proof in p.sp_global_dleq_proofs.items():
                ...
                _sim_verify_dleq(sim_exec, sim_execfile,
                                 summed_pubkey, scan_key, ecdh_share, proof, expected=True)

            for inp_idx, inp in enumerate(p.inputs):
                if not inp.sp_dleq_proofs:
                    continue
                pubkey = _sim_get_input_pubkey(sim_exec, sim_execfile, p, inp_idx)
                if pubkey is None:
                    continue
                for scan_key, proof in inp.sp_dleq_proofs.items():
                    ...
                    _sim_verify_dleq(sim_exec, sim_execfile,
                                     pubkey, scan_key, ecdh_share, proof, expected=True)

    def test_valid_output_script_derivation(self, sim_exec, sim_execfile):
        vectors = load_test_vectors()['valid']
        for vi in range(13):
            vec = vectors[vi]
            print("Compute output script - %s" % (vec['description']))
            p = BasicPSBT().parse(vec['psbt'].encode())
            sp_outputs = [(o.sp_v0_info[:33], o.sp_v0_info[33:66], o.script)
                          for o in p.outputs if o.sp_v0_info and len(o.sp_v0_info) == 66]
            if not sp_outputs:
                continue
    ...
    def test_in_progress_vectors(self, sim_exec, sim_execfile):
        vectors = load_test_vectors()['valid']
        for vi in range(0, 18):
            vec = vectors[vi]
            print("Verify DLEQ - %s" % (vec['description']))
            p = BasicPSBT().parse(vec['psbt'].encode())
            assert p is not None
            sp_outs = [o for o in p.outputs if o.sp_v0_info]
            if 'no sp outputs' not in vec['description']:
                assert len(sp_outs) > 0
