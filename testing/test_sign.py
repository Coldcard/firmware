...
def test_speed_test(dev, taproot, fake_txn, is_mark3, is_mark4, start_sign, end_sign, need_keypress):
    import time
...
def test_io_size(request, use_regtest, decode_with_bitcoind, fake_txn, is_mark3, is_mark4,
                    start_sign, end_sign, dev, segwit, taproot, out_style, accept = True):
    ...
    psbt = fake_txn(num_in, num_out, dev.master_xpub, segwit_in=segwit, taproot_in=taproot, outstyles=[out_style])
...
@pytest.mark.parametrize('segwit', [True, False])
@pytest.mark.parametrize('taproot', [True, False])
def test_real_signing(fake_txn, use_regtest, try_sign, dev, num_ins, segwit, taproot, decode_with_bitcoind):
...
def test_bip143_attack_data_capture(num_utxo, segwit_in, taproot_in, try_sign, fake_txn, settings_set,
    ...
    psbt = fake_txn(1, num_utxo+3, segwit_in=segwit_in, change_outputs=range(num_utxo+2), taproot_in=taproot_in,
...
def test_render_outs(out_style, segwit, taproot, outval, fake_txn, start_sign, end_sign, dev):
    # check how we render the value of outputs
    # - works on simulator and connected USB real-device
    xp = dev.master_xpub
    oi = int(Decimal(outval) * int(1E8))

    psbt = fake_txn(1, 2, dev.master_xpub, segwit_in=segwit, taproot_in=taproot, outvals=[oi, 1E8-oi],
...
def test_own_utxo_missing(segwit_in, taproot_in, num_missing, dev, fake_txn, start_sign, cap_story, end_sign, need_keypress):
    def hack(psbt):
...
def _test_single_sig_sighash(microsd_wipe, microsd_path, goto_home, cap_story, need_keypress,
                             bitcoind, bitcoind_d_sim_watch, settings_set, bitcoind_d_wallet_w_sk):
    def doit(addr_fmt, sighash, num_inputs=2, num_outputs=2, consolidation=False, sh_checks=False):
        ...
        ab = bitcoind.supply_wallet.getnewaddress()
        bitcoind.supply_wallet.generatetoaddress(1, ab)  # mine
        ...
        res = bitcoind_d_wallet_w_sk.testmempoolaccept([tx_hex])
        assert res[0]["allowed"]
        txn_id = bitcoind_d_wallet_w_sk.sendrawtransaction(tx_hex)
        assert tx_id == txn_id
...
def test_taproot_keyspend(use_regtest, bitcoind_d_sim_watch, start_sign, end_sign, microsd_path, cap_story, goto_home,
                          need_keypress, pick_menu_item, bitcoind):
    use_regtest()
