...
from chains import disassemble_multisig, BitcoinMain, BitcoinTestnet
...
if 1:
    ...
    M, N, fnd = disassemble_multisig(script, pk)
...
#                    'myY1Xmhx6CdvFn6uzdUDo5EM2HxmsPXPJB', 'mozpwp3z32g9vBZxbpN6ySxx7A5EWw4Zfi']
    ...
    addr, m_n, mine = BitcoinMain.p2sh_address(AF_P2SH, script, node=None)
    assert addr[0] == '3'
    assert m_n == (2, 10)
    addr, m_n, mine = BitcoinTestnet.p2sh_address(AF_P2SH, script, node=None)
    assert addr[0] == '2'

    addr, m_n, mine = BitcoinMain.p2sh_address(AF_P2WSH, script, node=None)
    assert addr[0:3] == 'bc1'
    assert len(addr) >= 62
    assert m_n == (2, 10)

    addr, m_n, mine = BitcoinTestnet.p2sh_address(AF_P2WSH, script, node=None)
    assert addr[0:3] == 'tb1'
    assert len(addr) >= 62
    assert m_n == (2, 10)
